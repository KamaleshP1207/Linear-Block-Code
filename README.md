...
Linear Block Code (LBC) is an error control coding technique used in digital communication to detect and correct errors during data transmission. In LBC, a block of `k` message bits is converted into an `n`-bit codeword by adding `(n-k)` parity bits.

The generator matrix is used to generate codewords:
...
The parity check matrix is used for error detection:
...
If the syndrome is zero, no error is present; otherwise, an error is detected and corrected. Linear Block Codes improve reliability and are widely used in digital communication systems.
# Program
```
import numpy as np
...
```
# Output Waveform
<img width="491" height="652" alt="image" src="https://github.com/user-attachments/assets/c7c398d9-9161-4d7a-ad54-ffb69c564d48" />
